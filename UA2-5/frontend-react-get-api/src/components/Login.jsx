...
import api from '../api'; // api axio que faz o acesso ao backend
import { UserContext } from '../App';

export default function Login() { // rota de login
  const navigate = useNavigate(); //hook que permite a navegação quando autenticado

  const { setUserName, setJWTAuth } = useContext(UserContext);

  async function handleLogin() { //função que faz o login - assicrona
    try {
      const body = {
      ...
      document.getElementById('mensagem-erro').innerHTML = data.message; //injeta a informação direta do back-end do erro
    }
  };
